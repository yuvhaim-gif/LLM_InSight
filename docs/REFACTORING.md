...
5. Internal optimizations: incremental token usage merging via `_merge_token_usage`, thread-local session ID cache in `state.py`, module-level `ERROR_PREFIXES` constant and pre-compiled regex patterns.
