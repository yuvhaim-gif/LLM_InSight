...
from ai.iteration_summary import resolve_ties_and_save, print_model_usage_summary, print_final_summary
from utils.session_keys import SK_PROMPT_HISTORY
...
def iterative_loop(prompt: str, min_grade: float, max_iterations: int = 5, 
    ...
    session[SK_PROMPT_HISTORY] = prompt_history
    ...
    tied_best_entries, cache_data = resolve_ties_and_save(all_entries, best_score_so_far, best_best_entry, current_prompt_num)

    print_model_usage_summary(current_prompt_num, iteration_count, iteration_models_used)

    print_final_summary(current_prompt_num, iteration_count, iteration_scores, ab_test_results, tied_best_entries, best_score_so_far, best_best_entry, last_iteration_best, session_weights)
